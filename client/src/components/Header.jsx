...
const BurgerMenu = ({ currentPage, setCurrentPage, width }) => {
  ...
  return (
    <>
    {width < 768 && <IconContainer>
      <MenuIcon style={iconStyling} onClick={() => handleToggle()}/>
    </IconContainer>}
    <MenuContainer open={width > 768 || open} width={width}>
      <MenuItem selected={currentPage === "LowrieSings"} onClick={() => handleSelection('LowrieSings')}>Home</MenuItem>
      <MenuItem selected={currentPage === "Song List"} onClick={() => handleSelection('Song List')}>Song list</MenuItem>
      <MenuItem selected={currentPage === "FAQs"} onClick={() => handleSelection('FAQs')}>FAQs</MenuItem>
      <MenuItem selected={currentPage === "Prices"} onClick={() => handleSelection('Prices')}>Prices</MenuItem>
      <MenuItem selected={currentPage === "Contact"} onClick={() => handleSelection('Contact')}>Contact</MenuItem>
    </MenuContainer>
    </>
  )
}
...
const Header = ({ currentPage, setCurrentPage }) => {
  const { width } = useWindowDimensions();
	return (
      ...
      <BurgerMenu currentPage={currentPage} setCurrentPage={setCurrentPage} width={width} />
		</Container>
	)
}
